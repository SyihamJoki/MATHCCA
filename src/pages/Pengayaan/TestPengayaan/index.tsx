...
import { pengayaan } from '../../../data/data';
import SweetAlert from 'react-native-sweet-alert';
import Sound from 'react-native-sound';

const TestPengayaan = ({ route, navigation }) => {
  const windowWidth = Dimensions.get('window').width;
  const windowHeight = Dimensions.get('window').height;
  const { user, onUpdate } = route.params;
  const data = pengayaan;
  ...
  useEffect(() => {
      if (data) {
          setQuestions(data);
          setSelectedAnswers(data.map(() => null));
  ...
  const handleSave = () => {
      if (!selectedAnswers[currentQuestionIndex]) {
          ...
          if (currentQuestionIndex === questions.length - 1) {
              navigation.replace("ScorePengayaan", {
                  totalPoints,
                  user: user,
                  onUpdate
              });
  ...
  if (loading) {
      ...
      return (
          <ContainerWithHeader title={"Pengayaan"}>
                      ...
                      <View style={[styles.card, { width: windowWidth * 0.8, borderRadius: 10, marginBottom: windowHeight * 0.02, height: windowHeight * 0.4 }]}>
                          {currentQuestion.questionText && (
                              <ScrollView style={{ maxHeight: currentQuestion.questionImage ? windowHeight * 0.15 : windowHeight * 0.4 }}>
                                  <Text style={[TextStyles.boldSmall, { color: 'white' , textAlign:'center'}]}>
                                      {currentQuestion.title}
                                  </Text>
                                  <Text style={[TextStyles.regularSmall, { color: 'white', textAlign:'justify' }]}>
                                      {currentQuestion.explain}
                                  </Text>
                                  <Text style={[TextStyles.regularSmall, { color: 'white', marginTop:5 }]}>
                                      {currentQuestion.questionText}
                                  </Text>
                              </ScrollView>
                          )}
