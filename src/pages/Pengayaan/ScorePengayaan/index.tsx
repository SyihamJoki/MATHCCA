...
const ScorePengayaan = ({route, navigation}) => {
  ...
  const saveTotalPointToFirestore = async (uid) => {
      try {
          ...
          if (docSnapshot.exists()) {
              ...
              // Update the exerciseType field with totalPoints
              await updateDoc(docRef, {
                  pengayaan: totalPoints, // Use the dynamic field name
              });
  
              // Update local user object with the new exerciseType
              let updatedUser = {
                  ...userData,
                  pengayaan: totalPoints,
              };  
...
return (
  ...
  <Loading visible={loading}/>
    <View>
      <Text style={[TextStyles.regularLarge,{color:'white', textAlign:'center', marginBottom:10}]}>Selamat anda telah menyelesaikan pengayaan.</Text>
      <ImageBackground source={require('../../../assets/image/bg-circle.png')} resizeMode='contain' style={{width:windowWidth*0.9, height:windowWidth*0.9, alignItems:'center', justifyContent:'center'}}>
        <Text style={[TextStyles.bold, {fontSize:45, color:'#4F2305'}]}>{totalPoints*20}</Text>
      </ImageBackground>
      <View style={{flexDirection:'row', width:windowWidth*0.82, alignSelf:'center', justifyContent:'space-between', marginTop: windowWidth*0.05}}>
        ...
        <View style={[styles.card, {width:windowWidth*0.4, height:windowHeight*0.12}]}>
          <Text style={[TextStyles.bold, {color:'white', fontSize:24}]}>{5-totalPoints}</Text>
          <View style={{flexDirection:'row', justifyContent:'center'}}>
            <View style={{justifyContent:'center'}}>
